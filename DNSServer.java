...
public class DNSServer {
    ...
    private HashMap<Integer, DatagramPacket> pendingQueries;
    ...
    public DNSServer(DNSZone zone) {
        ...
        // set the upstream server to be 127.0.0.53
        // note: I am assuming this is running on a recent Ubuntu system
        //       and I am using the Ubuntu server as my upstream
        try {
        ...
        pendingQueries = new HashMap<Integer, DatagramPacket>();
    }
    ...
    private DatagramPacket handleQuery(DNSMessage query) {
        ...
        pendingQueries.put(query.getID(), query.getPacket());
    ...
    private DatagramPacket handleReply(DNSMessage reply) {
        ...
        DatagramPacket originalQueryPacket = pendingQueries.remove(reply.getID());
        if (originalQueryPacket == null) {
            return null;
        }

        // add answers to the cache
        cache.addRecords(reply.getAnswers());

        // print the reply message again for consistency
        System.out.println("Forwarding Reply to " + originalQueryPacket.getSocketAddress());
        System.out.println(reply);

        //  make and return a new response packet to send to the original client
        return new DatagramPacket(reply.getData(), reply.getDataLength(), originalQueryPacket.getSocketAddress());
    }
    ...
    private DatagramPacket handleMessage(DatagramPacket incomingPkt) {
        ...
        // DNS Message object that will parse the request packet data
    ...
    /**
     * socket to receive UDP packets and handle those packets
     */
    public void run() {
        // ensure it will close when the try block finishes
        try (
